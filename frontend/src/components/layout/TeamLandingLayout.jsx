...
import { BookOpen, CheckCircle, GraduationCap, KeyRound, LayoutDashboard, ListChecks, LogOut, Menu, MessageSquareQuote, Users } from 'lucide-react'
...
export default function TeamLandingLayout() {
  ...
  useEffect(() => {
    try {
      const s = Number(sessionStorage.getItem('seen_total_team') || 0)
  ...
  useEffect(() => {
    ...
    if (!path.startsWith('/team/approvals')) return
    try {
      sessionStorage.setItem('seen_total_team', String(badgeTotal || 0))
  ...
  function getLinkBadgeCount(path) {
    if (!path) return 0
    if (path.startsWith('/team/approvals')) return badgeTotal || 0
    return 0
  }
          ...
          >
            ...
            <div className="gap-2 grid px-2 pt-2">
              ...
              <Button asChild variant="secondary" className="w-full" onClick={() => setOpen(false)}>
                <Link to="/team/profile">{t('dashboard.titles.profile')}</Link>
              </Button>
