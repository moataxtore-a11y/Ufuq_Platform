  ...
  function WalletBadge({ className }) {
    return (
      <button
        ...
        className={cn(
          'group inline-flex items-center justify-center gap-2 p-1 ps-3.5 pe-1 h-9 rounded-full transition-all duration-300 shrink-0 select-none border border-teal-400/40 shadow-sm hover:shadow-md hover:shadow-teal-950/40 hover:scale-[1.02] active:scale-[0.98]',
              ...
              <div className="items-center grid grid-cols-3">
                <div className={cn('flex items-center gap-2', isRtl ? 'justify-end flex-row-reverse' : 'justify-start flex-row')}>
                  <Link to="/student/profile" className="flex items-center justify-center shrink-0 rounded-full w-8 h-8 overflow-hidden border-2 border-slate-200 dark:border-white/20">
                    <img src={avatarUrl || defaultProfileAvatar} alt={displayName || (isRtl ? 'المستخدم' : 'User')} className="w-full h-full object-cover scale-120 translate-y-1" />
                  </Link>
