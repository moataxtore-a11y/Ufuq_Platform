...
function StatItem({ target, label, suffix = '+' }) {
  ...
  return (
    <div ref={ref} className="flex flex-col justify-center items-center px-[clamp(0.25rem,2vw,1.5rem)] py-1">
      <span className="font-black text-white text-[clamp(1rem,3.5vw,2.7rem)] tracking-tight drop-shadow-sm">
        {suffix}{count}
      </span>
      <span className="mt-1 font-bold text-teal-100 dark:text-teal-200 text-[clamp(0.6rem,1.5vw,1rem)]">
        {label}
      </span>
    </div>
  )
...
export default function HeroSection() {
  ...
  return (
    <section
      className="relative left-1/2 right-1/2 -ml-[50vw] -mr-[50vw] w-screen -mt-[68px] sm:-mt-[72px] md:-mt-[76px] pt-20 sm:pt-24 lg:pt-28 flex flex-col lg:h-[90vh] overflow-hidden bg-gradient-to-br from-[#4ee3d0] via-[#6cecd9] to-[#c2f6ef] dark:from-[#052622] dark:via-[#083832] dark:to-[#0c443e]"
      dir={isRtl ? 'rtl' : 'ltr'}
    >
      ...
      <div className="relative z-10 flex flex-col mx-auto px-[clamp(0.75rem,3vw,3rem)] pt-[clamp(0.5rem,1.5vw,1rem)] w-full max-w-[90rem]">
        ...
        <div className="items-center gap-[clamp(0.5rem,3vw,2.5rem)] xl:gap-16 grid grid-cols-12 h-[clamp(300px,min(52vw,calc(90vh_-_280px)),680px)] lg:h-[clamp(300px,min(52vw,calc(90vh_-_260px)),700px)] min-h-0">
          ...
          <div className="flex flex-col col-span-6 justify-center items-start space-y-[clamp(0.5rem,1.5vw,1.25rem)] z-20 order-1 lg:px-4 xl:px-6 min-w-0 text-start">
            ...
            <h1 className="font-perfect font-black text-slate-950 dark:text-teal-200 text-[clamp(1.75rem,7.2vw,5.4rem)] leading-[1.08] tracking-tight whitespace-nowrap drop-shadow-sm">
            ...
            <div className="space-y-1 w-full font-bold text-slate-900 dark:text-slate-100 text-[clamp(0.68rem,1.8vw,1.3rem)] leading-[1.65]">
              <p className="md:whitespace-nowrap drop-shadow-sm">
                منصة متكاملة بها كل ما يحتاجه الطالب ليتفوق
              </p>
              <p className="font-semibold text-slate-800 dark:text-slate-200 text-[clamp(0.6rem,1.8vw,1.125rem)]">
        ...
        <div className="relative z-30 flex justify-center -mt-[clamp(0.75rem,2.5vw,2.25rem)] mb-[clamp(1rem,2vw,1.5rem)] w-full">
          <div className="bg-[#0c6b73]/85 dark:bg-[#07474d]/90 shadow-[0_20px_50px_rgba(12,107,115,0.35)] hover:shadow-[0_25px_60px_rgba(12,107,115,0.45)] backdrop-blur-md px-[clamp(0.75rem,3vw,2rem)] lg:px-10 py-[clamp(0.75rem,1.6vw,1.25rem)] lg:py-4 border border-white/30 dark:border-teal-500/30 rounded-[clamp(1rem,3vw,2rem)] w-full max-w-4xl lg:max-w-[50rem] 2xl:max-w-[72rem] transition-all duration-300">
